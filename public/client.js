...
const addParticipants = ({ users }) => {
  let message = ''
  if (users === 1) {
    message += `Пока в чате один пользователь`
  } else {
    message += `Количество пользователей в чате - ${users}`
  }
  log(message)
}
...
S.on('user joined', (data) => {
  log(`${data.username} присоединился к беседе`)
  addParticipants(data)
})
...
S.on('disconnect', () => {
  log('Вы отключились от беседы')
})

S.on('reconnect', () => {
  log('Вы переподключились к беседе')
  if (username) {
    S.emit('add user', username)
  }
})
...
S.on('reconnect_error', () => {
  log('Проблемы с подключением к беседе')
})